  ...
  return (
    <section id="features" className="py-16 md:py-24 relative overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div className="absolute bottom-1/4 left-0 w-64 h-64 bg-accent/10 rounded-full blur-3xl animate-float-slow" />
      </div>
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-4 animate-slide-up" style={{ animationFillMode: 'both' }}>
            ...
            <span className="animate-text-shimmer">modern retail</span>
          </h2>
        ...
        <Tabs defaultValue="checkout" className="w-full animate-slide-up" style={{ animationDelay: '0.2s', animationFillMode: 'both' }}>
          <TabsList className="flex flex-wrap justify-center gap-2 bg-transparent h-auto mb-8">
            {features.map((feature, index) => (
              <TabsTrigger
                key={feature.id}
                value={feature.id}
                className="data-[state=active]:bg-primary data-[state=active]:text-primary-foreground px-3 py-2 rounded-full border border-border data-[state=active]:border-primary hover:scale-105 hover:border-primary/50 transition-all duration-300 text-sm group"
                style={{ animationDelay: `${index * 0.05}s` }}
              >
                <feature.icon className="w-4 h-4 mr-1.5 group-hover:animate-bounce-gentle" />
                {feature.title}
              </TabsTrigger>
            ))}
          </TabsList>
          ...
          {features.map((feature) => (
            <TabsContent key={feature.id} value={feature.id} className="mt-0">
              <div className="bg-card rounded-2xl p-6 md:p-8 border border-border hover:border-primary/30 transition-colors duration-300 hover:shadow-xl">
                <div className="grid md:grid-cols-2 gap-8 items-center">
                  <div className="animate-slide-in-left" style={{ animationFillMode: 'both' }}>
                    <div className="w-16 h-16 bg-primary/10 rounded-2xl flex items-center justify-center mb-6 hover:scale-110 hover:rotate-6 transition-all duration-500 group">
                      <feature.icon className="w-8 h-8 text-primary group-hover:animate-bounce-gentle" />
                    </div>
                    ...
                    <ul className="space-y-3">
                      {feature.details.map((detail, idx) => (
                        ...
                        >
                          <div className="w-5 h-5 rounded-full bg-primary/20 flex items-center justify-center mt-0.5 flex-shrink-0 group-hover:bg-primary/40 group-hover:scale-110 transition-all duration-300">
                            <div className="w-2 h-2 rounded-full bg-primary" />
                          </div>
                          <span className="text-foreground group-hover:text-primary transition-colors duration-300">{detail}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
                  <div className="relative rounded-xl overflow-hidden min-h-[300px] group animate-slide-in-right" style={{ animationFillMode: 'both' }}>
                    <img 
                      src={feature.image} 
                      alt={feature.title}
                    ...
                    {/* Floating badge */}
                    <div className="absolute bottom-4 left-4 bg-card/90 backdrop-blur-sm border border-border rounded-lg px-3 py-2 animate-float">
